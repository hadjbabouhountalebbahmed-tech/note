...
const CollapsibleSection: React.FC<CollapsibleSectionProps> = ({ title, children, isOpen, onToggle, isFilled, isHighlighting = false }) => {
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-200 dark:border-slate-700 overflow-hidden transition-all duration-300 ${isHighlighting ? 'ring-2 ring-teal-500 ring-offset-4 ring-offset-slate-50 dark:ring-offset-slate-900' : ''}`}>
      <button
        onClick={onToggle}
